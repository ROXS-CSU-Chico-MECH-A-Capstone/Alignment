...
class Linear: 
    ...
    def get(self,x): 
        ...
        if len(SA) <= 1:
            ...
            print(Ldata)
            print('_________SLAP Report:_________')
...
#for mock testing intensity not currently used
...
class Robot:
    ...
    robots = {}
    #ErrorData=[([300., 400.]), [(0.6980731626346021, -0.9085086590094147, 3.0), (0.0971312104853158, -1.756109836065993, 0.0)], [(-0.9035361576310216, -1.390522552554445, 5.0), (-2.006296278011765, -0.15869570068187894, -1.0)]]
    ErrorData=[[100., 250., 400.], [(0.644958119429604, 0.9590901298051105, 3.0), (1.3168924862121982, -0.9007453979775575, 3.0), (0.0971312104853158, -1.756109836065993, -4.0)], [(-0.729059710607757, -3.6464236693516536, -4.0), (-0.9035361576310216, -1.390522552554445, 7.0), (-1.5058472743484075, -0.0713713416656649, 3.0)]]
    d=None
    ...
    def e2z(cls,hv):
        ...
        FL=1000
        theta=np.arcsin(12398.5/(2*cls.d*hv))
        print(theta)
        z=2*FL*np.cos(theta)
        if z >= 100 and z<=500:
            print('z is')
    ...
    @classmethod
    def setd_API(cls,**kwargs):
        cls.setd(str(kwargs['material']), float(kwargs['h']), float(kwargs['k']),float(kwargs['l']))
        
    @classmethod
    def setd(cls,material,h,k,l):
        mat={'ge':5.657906,'si':5.431020}
        
        try:
            a0=mat[material]#*10**-10
            cls.d=a0/(h**2+k**2+l**2)**.5
            print('cls d is '+str(cls.d))
            return cls.d
        except:
            print('unsupported material type')
            print('Please use si or ge')
            return None
        
    
    @classmethod
    def SetRowPos_mm_API(cls,**kwargs):
        cls.SetRowPos_mm(float(kwargs['pos']))
        
    @classmethod
    def SetRowPos_API(cls,**kwargs):
        pos=cls.e2z(float(kwargs['hv']))
        print(pos)
        cls.SetRowPos_mm(pos)
        
    @classmethod
    def RSetRowPos_API(cls,**kwargs):
        pos=cls.e2z(float(kwargs['hv']))
        print(pos)
        cls.RSetRowPos_mm(pos)
        
    @classmethod
    def RSetRowPos_mm_API(cls,**kwargs):
 
        cls.RSetRowPos_mm(float(kwargs['pos']))
    
    @classmethod
    def SetRowPos_mm(cls,pos):
        #def RunRowland (Error,zi,zf,points):
        ESP=Webserver('192.168.0.99','values')
        R1=Robot('1 Mecademic Meca500 R3','192.168.0.100','1 CrystalToolSample','TPP1','APP1','LEDT1','Home1',ESP,'R1')
        R2=Robot('2 Mecademic Meca500 R3','192.168.0.101','2 CrystalToolSample','TPP2','APP2','LEDT2','Home2',ESP,'R2')
        
        
        ERL1=cls.ErrorReg(cls.ErrorData,1)
        ERL2=cls.ErrorReg(cls.ErrorData,2)
        
        Rlist1=cls.BuildRowScanADV(ERL1,pos,pos,1)
        Rlist2=cls.BuildRowScanADV(ERL2,pos,pos,1)
        #Glist=np.linspace(pos,pos,1)
    ...
    @classmethod
    def SLAP_ALL_I_mm_API(cls, **kwargs):
        
        runtime,cls.ErrorData=cls.SLAP_ALL_I(float(kwargs['zi']),float(kwargs['zf']), int(kwargs['loops']), int(kwargs['tests']))
      
    @classmethod
    def SLAP_ALL_I_API(cls, **kwargs):
    
        if cls.d == None:
            print('d not set :(')
            
        zi = cls.e2z( float(kwargs['ei']))
        zf = cls.e2z(float(kwargs['ef']))
        runtime,cls.ErrorData=cls.SLAP_ALL_I(zi,zf, int(kwargs['loops']), int(kwargs['tests']))
        print('###############')
        print(cls.ErrorData)

    @classmethod
    def SLAP_ALL_I(cls,zi,zf,loops,tests):
        #ERROR map for all robots
        #If it fails or takes a long time consider tuning of set Targets
        global Error
        cls.ErrorData=[ [] for _ in range(3)]
        ...
        for R in cls.robots.values():  #home everyone
            print(R)
            R.Connect_and_Home()
        i=0
        ...
        for R in cls.robots.values(): 
            ...
            for Etests in (range(0,tests)):
                ...
                time, coord = R.SLAP( ZL[Etests] , loops, 1 )
                print(coord)
                #Everything to do after an alignment
                
                cls.ErrorData[i].append(coord)
                print(cls.ErrorData)
                time=toc()
                Time.append(time)
                print('Time'+str(time))
        ...
        return Time,cls.ErrorData    
    
    @classmethod
    def RunRowland_mm_API(cls, **kwargs):
        zi = float(kwargs['zi'])
        zf = float(kwargs['zf'])
    ...
    def RunRowland(cls, zi, zf, points):
        ...
        Rlist1=cls.BuildRowScanADV(ERL1,zi,zf,points)
        Rlist2=cls.BuildRowScanADV(ERL2,zi,zf,points)
    ...
    @classmethod   
    def BuildRowScanADV(cls,ERL,zi,zf,points):
    ...
    def SetTargets(cls, PR_pos):
        ...
        RB1 = RDK.Item('1 Mecademic Meca500 R3 Base',ITEM_TYPE_FRAME) #position of robot 1 base
        RB2 = RDK.Item('2 Mecademic Meca500 R3 Base',ITEM_TYPE_FRAME) #position of robot 2 base
    ...
    def __init__(self,Name,IP,Tool,TPP,APP,LEDT,Home,ESP,webName):
        ...
        print(f'Initializing robot {self.webName}')
    ...
    def SLAPAPI(self, **kwargs):
        print(kwargs)
        self.SLAP(float(kwargs['PR_pos']), int(kwargs['loops']), int(kwargs['tests']))
    ...
    def PushPull_scan(self,data,step,coord,samples,prominence,width,height):
        #PUSH PULL test movement for SLAP
        tprint("PUSH PULL: \n-STARTED-",font='larry3d')
        xl,yl,zl=coord
        IPP=[]
        ZLPP=[]
    ...
    def SLAP_ALL(self,Robots,PR_pos,loops,tests):
        #TESTING software for SLAP
    ...
    def SLAP(self,PR_pos,loops,tests):
...
def DEMO():
    ...
    while True:
        ...
        Time,Error=R1.SLAP_ALL_I(Robots, PR_pos, 1, 3)
        print(Error)
...
Robot.ErrorData=[[300., 400.], [(0.6980731626346021, -0.9085086590094147, 3.0), (0.0971312104853158, -1.756109836065993, 0.0)], [(-0.9035361576310216, -1.390522552554445, 5.0), (-2.006296278011765, -0.15869570068187894, -1.0)]]
